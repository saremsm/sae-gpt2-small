...
with torch.no_grad():
    # checkpoint was trained on norm-sqrt(d) inputs (see training loop)
    x = (
        acts / acts.norm(dim=-1, keepdim=True).clamp(min=1e-8)
        * (cfg.d_model ** 0.5)
    )
    out = sae(x)

    resid_var = (x - out.reconstructed).var()
    total_var = x.var()
    ve = (1.0 - resid_var / total_var).item()
...
print(f"Variance explained   : {ve:.3f}  ({ve*100:.1f}%)")
